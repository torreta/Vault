...
int lector = 0,j=0;
int tamArchivo = 0,finarchivo=0,final=0,k=0,ack1=0,w=3;
...
//Lectura del archivo, se ignoran los \n
...
{
	 int j,malas,a,b,c,d,e,reenviar=0,ant=10;
   int conex;
   int ret,finalisimo=0;
   char* torre;
   /* se inicializa el cliente y espera por conexión*/
   conex = init_cliente(argc,argv);
   	printf("Conexion con -> %d\n", conex);
   
   //Almaceno toda la data del archivo
   leerArchivo("entradaC.txt");
   
   FILE *f_out = fopen("salida.txt","w");
   
   armarTrama(ack1);				//Armo la trama
   printf("Se enviará la trama -> %s\n", buf1);
   to_physical_layer(buf1, conex);			//Envio la trama
   while(TRUE)
   {
	//	 if(final!=1){
	//	   armarTrama();				//Armo la trama
	//	   printf("Se enviará la trama -> %s\n", buf1);
	//	   to_physical_layer(buf1, conex);			//Envio la trama
	//	 }
//				finalisimo=1;
//			}
     ret = from_physical_layer(buf2, conex);		//Recibo del servidor
     if(ret == FRAME){
...
//			printf("Se recibió la trama-> %s\n", buf2);
				memset(entrada,0,1000);
				if(malas!=0){
					reenviar=1;
	//				e=0;
	//				for(a=0;a<(ctoi(buf2[5])-malas);a++){
	//					for(d=0;d<20;d++){
